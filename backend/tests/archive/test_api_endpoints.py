...
print('-'*80)
try:
    from student_teams.documents.views import TeamDocumentListView
    
    factory = RequestFactory()
    request = factory.get('/api/teams/documents/')
    force_authenticate(request, user=student)
...
print('-'*80)
try:
    from repository.deliverables.views import CapstoneDeliverablesListView
    
    factory = RequestFactory()
    request = factory.get('/api/repository/deliverables/')
    force_authenticate(request, user=student)
