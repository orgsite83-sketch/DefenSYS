...
def extract_deliverables():
    ...
    for i, submission in enumerate(submissions, 1):
        ...
        if submission.extracted_text:
            print(f'   ⏭️  Already extracted ({len(submission.extracted_text)} chars)')
            skipped += 1
            continue
        
        # Check if file exists
        if not os.path.exists(submission.file.path):
            print(f'   ⚠️  File not found: {submission.file.path}')
            failed += 1
            continue
...
def extract_vault_entries():
    ...
    for i, entry in enumerate(entries, 1):
        ...
        if entry.extracted_text:
            print(f'   ⏭️  Already extracted ({len(entry.extracted_text)} chars)')
            skipped += 1
            continue
        
        # Check if file exists
        if not os.path.exists(entry.file.path):
            print(f'   ⚠️  File not found: {entry.file.path}')
            failed += 1
            continue
...
def extract_weekly_reports():
    ...
    for i, report in enumerate(reports, 1):
        ...
        if report.extracted_text:
            print(f'   ⏭️  Already extracted ({len(report.extracted_text)} chars)')
            skipped += 1
            continue
        
        # Check if file exists
        if not os.path.exists(report.report_file.path):
            print(f'   ⚠️  File not found: {report.report_file.path}')
            failed += 1
            continue
