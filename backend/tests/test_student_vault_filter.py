...
def test_student_vault_filtering():
    ...
    if not student_teams.exists():
        print("Warning: Student is not in any team - should see 0 entries")
        ...
        if len(payload['entries']) == student_vault_submissions.count():
            ...
            print(f"Warning: WARNING: Expected {student_vault_submissions.count()} entries, got {len(payload['entries'])}")
