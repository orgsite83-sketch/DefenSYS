...
class Rubric(models.Model):
    ...
    def context_label(self):
        if self.scope == self.SCOPE_PIT:
            return self.event_name or 'PIT Event'
    ...
    def clean(self):
        ...
        if self.scope == self.SCOPE_PIT:
            ...
            if not self.event_name.strip():
                errors['event_name'] = 'PIT rubrics require an event name.'
            if self.adviser_weight != 0:
                errors['adviser_weight'] = 'PIT adviser weight must be 0%.'

        if self.panel_weight + self.adviser_weight + self.peer_weight != 100:
