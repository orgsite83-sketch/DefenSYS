...
from capstone_deliverables.services import STAGE_OPTIONS, display_name
...
def _matches_search(entry, search):
    if not search:
        return True
    haystack = ' '.join(
        ...
        for key in [
            ...
            'stage',
            'uploaded_by',
            'extracted_text',  # Include PDF content in search
            'summary',  # Include summary in search
        ]
    ).lower()
...
def digital_vault_payload(request):
    ...
    filtered_entries = filter_entries(entries, request.query_params)
    return {
