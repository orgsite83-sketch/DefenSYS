...
class DigitalVaultApiTests(APITestCase):
    ...
    def test_all_roles_can_read_public_vault(self):
        for user in [self.admin, self.faculty, self.student]:
            ...
            response = self.client.get('/api/digital-vault/')
    ...
    def test_search_and_filters_apply_to_unified_entries(self):
        ...
        pit_response = self.client.get('/api/digital-vault/', {'type': 'pit', 'year_level': '3rd Year'})
        capstone_response = self.client.get('/api/digital-vault/', {'search': 'concept', 'stage': 'Concept Proposal'})
        empty_response = self.client.get('/api/digital-vault/', {'academic_year': '2025-2026'})
