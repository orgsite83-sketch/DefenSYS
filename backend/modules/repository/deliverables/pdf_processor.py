...
def extract_pdf_from_file_object(file_obj, file_name: str = '', classify: bool = False) -> Dict[str, any]:
    ...
    Works with local disk and S3-backed FileField (uses a temp file for pdfplumber).
    """
    suffix = '.pdf' if (file_name or '').lower().endswith('.pdf') else '.pdf'
    tmp_path = None
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
            ...
            tmp_path = tmp.name
        return extract_pdf_content(tmp_path, classify=classify)
    finally:
